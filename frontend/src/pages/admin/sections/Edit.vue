<template>
        ...
        <div class="bg-white shadow rounded-lg">
          <div class="px-6 py-4 border-b border-gray-200">
            <div class="flex items-center justify-between">
              <h1 class="text-2xl font-bold text-gray-900">Editar Seção</h1>
          ...
          <form @submit.prevent="submitForm" class="p-6 space-y-6" v-if="section">
            <!-- Nome -->
            <div>
            ...
            <div class="border-t border-gray-200 pt-6">
              ...
              <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div class="bg-gray-50 p-4 rounded-lg">
                  <div class="text-2xl font-bold text-blue-600">{{ section.articles_count || 0 }}</div>
                  <div class="text-sm text-gray-600">Artigos</div>
                </div>
                <div class="bg-gray-50 p-4 rounded-lg">
                  <div class="text-2xl font-bold text-green-600">{{ formatDate(section.created_at) }}</div>
                  <div class="text-sm text-gray-600">Criado em</div>
                </div>
                <div class="bg-gray-50 p-4 rounded-lg">
                  <div class="text-2xl font-bold text-purple-600">{{ formatDate(section.updated_at) }}</div>
            ...
            <div class="flex justify-between pt-6 border-t border-gray-200">
              ...
              <div class="flex space-x-3">
                ...
                >
                  {{ loading ? 'Salvando...' : 'Atualizar Seção' }}
                </button>
              </div>
            </div>
          </form>
          ...
          <div v-else class="p-6">
            <div class="text-center">
              ...
              <p class="mt-4 text-gray-600">Carregando seção...</p>
            </div>
          </div>
        </div>
      </div>
  ...
  export default {
    ...
    methods: {
      async loadSection() {
        try {
          const response = await fetch(`/api/sections/${this.$route.params.id}`)
          this.section = await response.json()
          ...
          this.form = {
            ...
            sort_order: this.section.sort_order || 0,
            meta_title: this.section.meta_title || '',
            meta_description: this.section.meta_description || '',
            color: this.section.color || '#3B82F6',
            icon: this.section.icon || '',
            cover_image: this.section.cover_image || ''
          }
        } catch (error) {
      ...
      async submitForm() {
        this.loading = true
        try {
          const response = await fetch(`/api/sections/${this.section.id}`, {
            method: 'PUT',
            headers: {
          ...
          if (response.ok) {
            ...
            console.error('Erro ao atualizar seção')
          }
        } catch (error) {
